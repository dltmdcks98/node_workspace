...
var mysql = require("mysql");//mysql관련 외부 모듈 //npm install mysql 콘솔에 실행
//서버 객체 생성 
...
function insert(request, response) {
    ...
    request.on("end", function () {//모든 데이터를 다 받은 후 마지막 동작
        ...
        //mysql 접속
        var con = mysql.createConnection({//DriverManger.getConnection();과 같음 
            url: "localhost",
            user: "root",
            password: "1234",
            database: "javastudy"
        ...
        con.query(sql, [result.title, result.writer, result.content], function (error, results, fields) {//쿼리문 수행
            if (error) {

                //등록 실패
                response.writeHead(200, { "Content-type": "text/html;charset=utf-8" });//헤더 정보 구성
                response.end("글쓰기 에러");//body
            } else {
                //등록 성공
                response.writeHead(200, { "Content-type": "text/html;charset=utf-8" });//헤더 정보 구성
